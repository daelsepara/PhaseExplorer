...
public partial class MainWindow : Gtk.Window
{
    ...
    void OnRemovePointButtonClicked(object sender, EventArgs args)
    {
        ...
        PointsView.Buffer.GetSelectionBounds(out start, out end);
    ...
    protected void OnInputWidthValueChanged(object sender, EventArgs e)
    {
        if (!ControlsEnabled)
            return;

        if (InputWidth.Value >= Parameters.WindowX)
        {
            Parameters.SLModX = Convert.ToInt32(InputWidth.Value);
    ...
    protected void OnInputHeightValueChanged(object sender, EventArgs e)
    {
        if (!ControlsEnabled)
            return;

        if (InputHeight.Value >= Parameters.WindowY)
        {
            Parameters.SLModY = Convert.ToInt32(InputHeight.Value);
